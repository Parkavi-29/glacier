...
current\_time\_ist = datetime.now(ist).strftime('%Y-%m-%d %H:%M:%S').upper()
...
user\_q = st.text\_input("Your question:", placeholder="e.g. What is NDSI?")
if user\_q:
q = user\_q.lower()
if "ndsi" in q:
...
csv\_url = '[https://raw.githubusercontent.com/Parkavi-29/glacier/main/Gangotri\_Glacier\_Area\_NDSI\_2001\_2023.csv](https://raw.githubusercontent.com/Parkavi-29/glacier/main/Gangotri_Glacier_Area_NDSI_2001_2023.csv)'
try:
df = pd.read\_csv(csv\_url)
df = df.dropna(subset=\['year', 'area\_km2'])
...
if df is not None:
year\_min, year\_max = int(df\['year'].min()), int(df\['year'].max())
year\_range = st.slider("📆 Select year range:", year\_min, year\_max, (year\_min, year\_max), step=1)
df\_filtered = df\[(df\['year'] >= year\_range\[0]) & (df\['year'] <= year\_range\[1])]
...
st.markdown(""" <div style="border: 2px solid #0b3954; padding: 20px; border-radius: 10px; background-color: rgba(255, 255, 255, 0.95); font-family: 'Catamaran', sans-serif;"> <h3 style="color: #0b3954;">📍 Area of Interest (AOI) Summary</h3> <ul style="line-height: 1.7; font-size: 16px;"> <li><b>Total Glacier Area:</b> \~64.13 sq.km</li> <li><b>Bounding Box:</b> <ul> <li>Longitude: 79.03°E → 79.10°E (\~7.7 km)</li> <li>Latitude: 30.94°N → 31.02°N (\~8.9 km)</li> </ul> </li> <li><b>Major Places Covered:</b> <ul> <li>Gangotri Glacier</li> <li>Gaumukh Snout</li> <li>Chirbasa & Bhojbasa</li> <li>Tapovan (partial)</li> <li>Gangotri National Park</li> </ul> </li> </ul> </div>
